...
describe('AzureRetailService', () => {
  ...
  it('should write files to directory on fetching data from service that returns only one page and Config enabled to wirte to file/store ', () => {
    ...
    const parseProductInPage = jest.spyOn(ProductHelperService.prototype, 'parseProductInPage').
    service.downloadAzureRetialFiles();
    expect(storeToFileMock).toHaveBeenCalledTimes(2);
    expect(httpService.get).toHaveBeenCalledTimes(1);
    expect(fs.writeFile).toHaveBeenCalledTimes(1);
    expect(mockproductHelperService.parseProductInPage).not.toHaveBeenCalled();
  });
