...
import { defer, mergeMap, Observable, of, tap, timer } from 'rxjs';
...
export class AzureRetailService {
  ...
  private fetchPage(currentLink: string, pagenumber: integer) {
    return this.httpService.get(currentLink).pipe(tap(() => console.log(`-> fetched page ${page}`)),
    map({
      items: Array.from()
    })
  }
