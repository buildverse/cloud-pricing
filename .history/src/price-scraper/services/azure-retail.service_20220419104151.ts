...
export class AzureRetailService {
  ...
  private fetchPage(currentPageLink: string): Observable<{ status: number; link: any | null; }> {
    if (currentPageLink == null) r Azure downloaded');
  ...
  downloadPrices() {
    ...
    const currentPageLink: string | null = null;
    const pageNumber = 1;
