...
import { defer, from, map, mergeMap, Observable, of, tap, timer } from 'rxjs';
...
export class AzureRetailService {
  ...
  private fetchPage(currentLink: string, pageNumber: number) {
    return this.httpService.get(currentLink).pipe(tap(() => console.log(`-> fetched page ${pageNumber}`)),
    map((response) => {
      items: response.data
      nextPage: pageNumber +1
    })
  }
  getFiles = page => defer(() => this.fetchPage(page)).pipe(
    mergeMap(({items, nextpage})=> {
      const items$ = from (items);
      const next$ = nextpage ? nextpage + 1 : EM
    })
  )
