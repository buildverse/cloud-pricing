...
export class AzureRetailService {
  ...
  fetchFile({ currentLink }: { currentLink: string; }): Observable<{}> {
    return this.httpService.get(currentLink).pipe(
      map((response) => {
        return {
          data: response.data,
          link: response.data.NextPageLink,
        };
      }),
    );
  }
  ...
  fetchAllFiles() {
    ...
    this.fetchFile({ currentLink: currentPageLink }).pipe(expand(response) => {
      return response.link ? this.fetchFile({ currentLink: response.link }) : EMPTY
    });
  }
