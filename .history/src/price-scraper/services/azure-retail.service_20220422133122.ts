...
export class AzureRetailService {
  ...
  persistFiles(filename: string, data: string): void {
    this.logger.debug('Saving to storage :' + this.configService.get('SAVE_CLOUD_PRICE_FILES_TO_STORAGE'));
    if( this.configService.get('SAVE_CLOUD_PRICE_FILES_TO_STORAGE')) {
      this.persisteFilesToStore()
    }
  }

  private persisteFilesToStore(filename: string, data: string){
