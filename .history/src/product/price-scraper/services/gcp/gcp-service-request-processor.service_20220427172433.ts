...
export class GCPServiceRequestProcessor {
  ...
  private fetchFile(
    ...
    return this.httpService.get(currentLink).pipe(
      map((response) => {
        this.logger.debug(`Reponse data is ${response.data}`);
  ...
  fetchServices(url: string): void { let nextPageParam = '';
    const nextPageToken = '';
    if (nextPageToken) {
      nextPageParam = `&pageToken=${nextPageToken}`;
    }
    ...
    this.logger.debug(`URL is ${}`)
    this.httpService
      .get(currentPageLink)
      .subscribe((response) => this.logger.debug(`Repsonse is ${response}`));
  }
}
