...
import { EMPTY, expand, from, map, Observable, of } from 'rxjs';
...
export class GCPServiceRequestProcessor {
  ...
  ): Observable<Observable<GCPService[]> | { data: string; nextPageToken: string; }> {
    let nextPageParam = '';
    let pageNumber = 1;
    const nextPageToken = '';
    if (nextPageToken) {
  ...
  private fetchDataSet(
    ...
    Observable<GCPService[]> | { data: string; nextPageToken: string }
  > {
    this.logger.info(
