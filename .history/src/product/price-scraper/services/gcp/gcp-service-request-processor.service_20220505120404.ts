...
export class GCPServiceRequestProcessor {
  ...
  fetchFiles(url: string, serviceDescription?: boolean): Observable<{ data: string; nextPageToken: string }> {
    let nextPageParam = '';
    let pageNumber = 1;
    const nextPageToken = '';
    if (nextPageToken) {
    ...
    const currentPageLink = `${this.getURL(${url}, ${serviceDescription})${nextPageParam}`;
