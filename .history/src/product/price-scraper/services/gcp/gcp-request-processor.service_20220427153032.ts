...
export class GCPRequestProcessor {
  ...
  fetchFiles(url: string, getServices?: boolean): Observable<{ data: string; link: string }> {
    let currentPageLink: string | null = null;
    let pageNumber = 1;
    if (!currentPageLink) {
      currentPageLink = `${url}`;
    }
    return this.fetchFile(currentPageLink, pageNumber, true).pipe(
      expand((response) => {
        if (response.link) {
