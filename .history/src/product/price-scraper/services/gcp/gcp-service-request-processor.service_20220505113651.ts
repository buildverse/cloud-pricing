  ...
  private fetchFile(
    ...
    return this.httpService.get(currentLink).pipe(
      map((response) => {
        ...
        //this.services = deserializeArray(GCPService, response.data.services);
        response.data.services.map(service => plainToInstance(GCPService, this.services));
        this.logger.debug(`service is  ${this.services.length}`);
        this.logger.debug(`service is  ${JSON.stringify(this.services)}`);
        return {
