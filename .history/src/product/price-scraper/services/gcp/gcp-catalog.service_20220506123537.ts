...
export class GcpCatalogService {
  ...
  private getGCPCatalogueData(): void {
    ...
    this.gcpServiceRequestProcessor
      ...
      .pipe(
        map((services) => serviceArray = [...serviceArray, plainToClass(GCPService, services.data, {excludeExtraneousValues: true})])).pipe()
        
        from(serviceArray).pipe(mergeMap(service => this.gcpServiceRequestProcessor.fetchData(this.baseUrl, true, service.serviceId)))
