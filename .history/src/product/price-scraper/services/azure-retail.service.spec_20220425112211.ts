...
//jest.mock('../../product-helper/product-helper.service');
...
describe('AzureRetailService', () => {
  ...
  it('should write files to directory on fetching data from service that returns only one page and Config enabled to wirte to file/store ', () => {
    jest
    ...
    const parseProductInPage = jest
      .spyOn(ProductHelperService.prototype, 'parseProductInPage')
      .mockImplementation(() => []);
    ...
    expect(storeToFileMockConfig).toHaveBeenCalledTimes(1);
    expect(httpService.get).toHaveBeenCalledTimes(1);
    expect(fs.writeFile).toHaveBeenCalledTimes(1);
    expect(parseProductInPage).not.toHaveBeenCalled();
    expect(createProductsMock).not.toHaveBeenCalled();
  });
  // @TODO: The commented expectation not wokring. Need to figure it out.
  it('should save to DB and not write to file system or storage, fetching data from service ', () => {
    ...
    expect(parseProductInPage).toHaveBeenCalledTimes(1);
  ...
  it('should write files to directory on fetching data from service that returns multiple pages ', () => {
    ...
    expect(storeToFileMockConfig).toBeCalledTimes(2); // will be called 2 times due to recursive natre
  });
});
