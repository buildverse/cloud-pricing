...
describe('ProductService', () => {
  ...
  it('should sucessfully save data to Database ', () => {
    ...
    const returnValue = jest.spyOn(service , 'createProducts').mockImplementation
    const result = service.createProducts(productDto);
    expect(result).not.toBeNull();
    expect(saveWithoutError).toHaveBeenCalledTimes(1);
    expect(result[0].updatedTime.getDate()).toBe(new Date().getDate());
  });

  it('should sucessfully handle error if save failed to Database ', () => {
    let result;
    const saveWithError = jest
      .spyOn(repository, 'save')
      .mockRejectedValueOnce(new Error('Unable to Save data in DB'));
    try {
      result = service.createProducts(productDto);
    } catch (e) {
      console.log('Error is ' + JSON.stringify(e));
      expect(result).not.toBeDefined();
      expect(saveWithError).toHaveBeenCalledTimes(1);
      expect(saveWithError).toThrow('Unable to Save data in DB');
    }
  });
});
