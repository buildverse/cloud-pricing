...
describe('ProductHelperService', () => {
  ...
  it('should create product entity from json payload', () => {
    const createdProduct = service.parseProductInPage(jsonDataLoad);
    expect(createdProduct.length).toBe(4);
    const products = jsonDataLoad.Items.forEach(item => {
      
    });
    expect(service.products).toEqual(jsonDataLoad.Items[0].serviceFamily,
    );
